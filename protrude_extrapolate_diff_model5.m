function [Im_new, Ps_new, P_new, Is_new, I_new, Cs_new, C_new, Rs_new, R_new] = protrude_extrapolate_diff_model5(Im, Ps, P, Is, I, Cs, C, Rs, R, F, g, k, V0, alpha_V, beta_V, gamma_V, A_act, alpha_A, beta_A, gamma_A)
    ...
    Ps_cor = extapolate_C(Ps, Im);
    Ps_ex = ex.*Ps_cor;
    Ps_new = Ps + Ps_ex;
    Ps_new = Ps_new - Im_new*sum(Ps_ex(:))/sum(Im_new(:));
    
    P_cor = extapolate_C(P, Im);
    P_ex = ex.*P_cor;
    P_new = P + P_ex;
    P_new = P_new - Im_new*sum(P_ex(:))/sum(Im_new(:));
